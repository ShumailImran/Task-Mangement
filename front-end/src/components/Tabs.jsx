        ...
        <Tab.List className="flex space-x-6 rounded-xl p-1">
          {tabs.map((tab, index) => (
            <Tab
              ...
              className={({ selected }) =>
                classNames(
                  "w-fit flex items-center outline-none gap-2 px-3 py-2.5 text-base font-medium leading-5 bg-white dark:bg-[#121212]",

                  selected
                    ? "text-blue-700 dark:text-blue-400  border-b-2 border-blue-600 dark:border-blue-400"
                    : "text-gray-800 dark:text-gray-400  hover:text-blue-800 dark:hover:text-blue-400"
                )
              }
            >
              {tab.icon}
